...
{func}`~datasets.preprocess.standardize`

: Standardize each time step to have zero mean and unit variance.

{func}`~datasets.preprocess.minmax_scale`

: Normalize each time step in a predefined range, by default between 0 and 1.

{func}`~datasets.preprocess.maxabs_scale`

: Scale each time step by the maximum absolute value,

{func}`~datasets.preprocess.truncate`

: Truncate each time series to have uniform length, i.e., to the length of the
shortest time series.

In contrast to
{doc}`feature-wise preprocessing <sklearn:modules/preprocessing>`, the
preprocessing operations in the {mod}`datasets`-module operate sample-wise and
are state-less, i.e., we can reuse them for both the training and testing parts
of our data. To simplify the application of preprocessing, the
{func}`~datasets.load_dataset` function accepts a `preprocess` parameter:
...
{func}`~datasets.preprocess.truncate` to accomplish this.
...
Since many algorithms in Wildboar only support dimensions and samples of uniform
length, we can preprocess the time series using the truncate function. One
should note that truncating is very crude and result in data loss.
