...
- [Install the latest official distribution](#install-the-latest-release) from
  [PyPi](https://pypi.org/project/wildboar). This is the recommended approach
  for most users.

- [Build and compile the package from source](#build-and-compile-from-source).
...
install `numpy==1.19` before installation.
```

To avoid conflicts with already installed packages, it is strongly recommended
installing the package in a
[virtual environment](https://docs.python.org/3/tutorial/venv.html). You can set
...
[Anaconda](https://docs.conda.io/en/latest) or
[Miniconda](https://docs.conda.io/en/latest/miniconda.html). `wildboar` is still
installed
using ``pip``
````
`````
...
1. Use [Git](https://git-scm.com/) to fetch the latest version.
...
2. Install a c-compiler for your operating system.
   ...
   First install [build tools for Visual Studio 2019](https://visualstudio.microsoft.com/downloads/)
...
5. Build the project in editable mode to ease development
   ...
   Setting `WILDBOAR_BUILD=optimized` can build a version optimized for the
